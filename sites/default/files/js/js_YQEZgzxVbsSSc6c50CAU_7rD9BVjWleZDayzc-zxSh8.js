...
$( document ).ready(function() {
    ...
    $("[href^='#']").click(function() {
      id=$(this).attr("href");
      var pos_id = $(id).offset().top;
      var scr_top = $(window).scrollTop();
      console.log("id: "+pos_id+"  window: "+scr_top)
      if(pos_id >= scr_top){
        $('html, body').animate({
            scrollTop: pos_id - 50
        }, 1000);
        console.log('down, scroll to:'+(pos_id - 50))
      }else{
        $('html, body').animate({
          scrollTop: pos_id - 165
        }, 1000);
        console.log('up, scroll to:'+(pos_id - 165))
      }
    });
